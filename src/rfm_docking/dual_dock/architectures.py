...
class DockCSPNet(CSPNet):
    def forward(
        ...
        com_batch = torch.repeat_interleave(
            torch.arange(batch.loading.shape[0], device=batch.xt.device),
            batch.loading,
        )

        # create graph
        # for osda
        com_edges, com_frac_diff = gen_edges(
            batch.loading,
            com_xt,
            batch.lattices,
            com_batch,
        ...
        com_edge_feats = dummy_edge_ids.repeat(com_edges.shape[1], 1)
        com_edge_feats = self.edge_embedding(com_edge_feats)
        ...
        cross_num_atoms = batch.loading + batch.zeolite.num_atoms
        ...
        is_com = [
            torch.cat(
                [
                    torch.ones(
                        batch.loading[i], dtype=torch.bool, device=com_xt.device
                    ),
                    torch.zeros(
                        batch.zeolite.num_atoms[i],
                        dtype=torch.bool,
                        device=com_xt.device,
                    ),
                ],
                dim=0,
            )
        ...
        is_com = torch.cat(is_com, dim=0)

        cross_frac_coords = [
            torch.cat(
                [
                    batch.xt[com_batch == i],
        ...
        com_to_zeolite_edges = is_com[cross_edges[0]] & torch.logical_not(
            is_com[cross_edges[1]]
        )
        zeolite_to_com_edges = is_com[cross_edges[1]] & torch.logical_not(
            is_com[cross_edges[0]]
        )

        is_cross_edge = com_to_zeolite_edges | zeolite_to_com_edges
        ...
        t_per_atom = t_emb.repeat_interleave(batch.osda.num_atoms, dim=0)
        be_per_atom = be_emb.repeat_interleave(batch.osda.num_atoms, dim=0)
        ...
        com_scatter_batch = torch.repeat_interleave(
            torch.arange(batch.loading.sum(), device=batch.xt.device),
            torch.repeat_interleave(
                batch.osda.num_atoms // batch.loading,
                batch.loading,
            ),
            dim=0,
        )
        com_node_features = scatter(
            osda_node_features,
            com_scatter_batch,
            dim=0,
            reduce="sum",
            # intuition why sum might work well: more atoms -> larger molecule
        )
        ...
        com_zeolite_merged = merge_edges(
            batch.loading,
            batch.zeolite.num_atoms,
            com_edges,
            zeolite_edges,
        )
        edges = torch.cat([com_zeolite_merged, cross_edges], dim=1)

        edge_feats = torch.cat(
            [com_edge_feats, zeolite_edge_feats, cross_edge_feats], dim=0
        )
        frac_diff = torch.cat(
            [com_frac_diff, zeolite_frac_diff, cross_frac_diff], dim=0
        )
        ...
        coord_out = self.coord_out(node_features[is_com])
        ...
        be_out = self.be_out(com_node_features)
